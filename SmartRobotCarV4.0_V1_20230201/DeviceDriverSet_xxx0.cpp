...
void DeviceDriverSet_ULTRASONIC::DeviceDriverSet_ULTRASONIC_Init(void)
{
  ...
  pinMode(LIGHT_PINLL, INPUT);
  pinMode(LIGHT_PINL, INPUT);
  pinMode(LIGHT_PINM, INPUT);
  pinMode(LIGHT_PINR, INPUT);
  pinMode(LIGHT_PINRR, INPUT);
  pinMode(PEWPEW_PIN, OUTPUT);
}
...
void DeviceDriverSet_ULTRASONIC::DeviceDriverSet_ULTRASONIC_GetRight(uint16_t *ULTRASONIC_Get /*out*/)
{
  unsigned int tempda_x = 0;
  ...
  delayMicroseconds(10);
  digitalWrite(TRIG_PIN2, LOW);
  tempda_x = ((unsigned int)pulseIn(ECHO_PIN2, HIGH) / 58);
  // *ULTRASONIC_Get = tempda_x;
