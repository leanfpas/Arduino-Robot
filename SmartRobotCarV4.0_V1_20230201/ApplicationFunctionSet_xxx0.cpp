...
void ApplicationFunctionSet::ApplicationFunctionSet_Tracking(void) {
  uint16_t get_Distance_Left; // Left Sensor Pointing Right
  uint16_t get_Distance_Right;  // Right Sensor Pointing Left
  int sensorLeft = digitalRead(SENSOR_PIN1);
  int sensorRight = digitalRead(SENSOR_PIN2);
  int get_R = digitalRead(RIGHT_PIN);
  int get_M1 = AppITR20001.DeviceDriverSet_ITR20001_getAnaloguexxx_M();
  int get_M2 = AppITR20001.DeviceDriverSet_ITR20001_getAnaloguexxx_L();
  int get_M3 = AppITR20001.DeviceDriverSet_ITR20001_getAnaloguexxx_R();
  int get_L = digitalRead(LEFT_PIN);
  int lightRight2 = digitalRead(LIGHT_PINRR);
  int lightRight = digitalRead(LIGHT_PINR);
  int lightMid = digitalRead(LIGHT_PINM);
  int lightLeft = digitalRead(LIGHT_PINL);
  int lightLeft2 = digitalRead(LIGHT_PINLL);

  AppULTRASONIC.DeviceDriverSet_ULTRASONIC_Get(&get_Distance_Right);
  // Retrieve the distance of the left sensor pointing
  AppULTRASONIC.DeviceDriverSet_ULTRASONIC_GetRight(&get_Distance_Left);
  // Retrieve the distance of the right sensor pointing left
  if (Application_SmartRobotCarxxx0.Functional_Mode == TraceBased_mode) {
    // IR/Edge Detection
    Serial.println(sensorLeft);
    Serial.println(sensorRight);
    // Ultrasonic/Obstacle Avoidance
    Serial.println(get_Distance_Left);
    Serial.println(get_Distance_Right);
    // Line Tracking Values
    Serial.print("ITR20001_getAnaloguexxx_L=");
    Serial.println(get_L);
    Serial.print("ITR20001_getAnaloguexxx_M=");
    Serial.println(get_M1);
    ...
    // if else cases for edge detection + obstacle avoidance:
    if (sensorLeft == 1) {
      ApplicationFunctionSet_SmartRobotCarMotionControl(Backward, 60);
      delay_xxx(400);
      ...
      delay_xxx(1500);
    //} else if ((get_Distance_Right < 12 || get_Distance_Left < 12) && lightMid == 0) {
      //ApplicationFunctionSet_SmartRobotCarMotionControl(stop_it, 60);
      //delay_xxx(500);
    } else if (lightMid == 0) {
      digitalWrite(PEWPEW_PIN, LOW);
      ApplicationFunctionSet_SmartRobotCarMotionControl(Left, 45);
      delay_xxx(300);
      ApplicationFunctionSet_SmartRobotCarMotionControl(Right, 45);
      delay_xxx(200);
    } else if (lightRight == 0 || lightRight2 == 0) {
      ApplicationFunctionSet_SmartRobotCarMotionControl(Right, 75);
    } else if (lightLeft == 0 || lightLeft2 == 0) {
      ApplicationFunctionSet_SmartRobotCarMotionControl(Left, 75);
    } else if (get_Distance_Right < 17) {
      ApplicationFunctionSet_SmartRobotCarMotionControl(Left, 70);
      delay_xxx(250);
    } else if (get_Distance_Left < 17) {
      ApplicationFunctionSet_SmartRobotCarMotionControl(Right, 70);
      delay_xxx(200);
    } else if (get_L == 1 && get_R == 1) {
      ApplicationFunctionSet_SmartRobotCarMotionControl(Left, 75);
    } else if (get_L == 1 && get_M1 > 250){
      ApplicationFunctionSet_SmartRobotCarMotionControl(Left, 100);
    } else if (get_R == 1 && get_M1 > 250){
...
void ApplicationFunctionSet::ApplicationFunctionSet_Obstacle(void) {
  ...
  if (Application_SmartRobotCarxxx0.Functional_Mode == ObstacleAvoidance_mode) {
    ...
    //} else if ((get_Distance_Right < 12 || get_Distance_Left < 12) && lightMid == 0) {
      ...
      ApplicationFunctionSet_SmartRobotCarMotionControl(Left, 75);
    } else if (get_Distance_Right < 15) {
      ApplicationFunctionSet_SmartRobotCarMotionControl(Left, 60);
      delay_xxx(250);
    } else if (get_Distance_Left < 15) {
      ApplicationFunctionSet_SmartRobotCarMotionControl(Right, 60);
      delay_xxx(200);
    } else if (get_L == 1 && get_R == 1) {
      ApplicationFunctionSet_SmartRobotCarMotionControl(Left, 75);
